...
class CSFramework_Option_upload extends CSFramework_Options {
  ...
  public function output() {
    ...
    $pram_type = ( isset( $pram_type ) ) ? $pram_type : null;

    echo '<input type="text" name="'. $this->element_name() .'" value="'. $this->element_value() .'"'. $this->element_class() . $this->element_attributes() .'/>';
    echo '<a href="#" class="button cs-add" data-frame-title="'. $frame_title .'" data-upload-type="'. $upload_type .'" data-pram-type="'. $pram_type .'" data-insert-title="'. $insert_title .'">'. $button_title .'</a>';
