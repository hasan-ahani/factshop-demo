...
class Button extends Widget_Base {
	...
	public function get_name() {
		return 'factshop-button';
	}
	
	public function get_title() {
		return __( 'دکمه', 'factshop' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'size',
			[
				'label' => __( 'اندازه', 'factshop' ),
				'type' => Controls_Manager::SELECT,
				'default' => '',
				'options' => [
					'btn-sm'  => __( 'کوچک', 'factshop' ),
					''  => __( 'متوسط', 'factshop' ),
					'btn-lg'  => __( 'بزرگ', 'factshop' )
				],
			]
		);
        $this->add_control(
	...
	protected function render() {
		...
		?>
		<a href="<?php echo esc_url($settings['button_link']['url']); ?>" class="btn btn-<?php echo esc_attr($settings['outline']) . esc_attr($settings['style']) . ' ' . esc_attr($settings['size']) .' '.esc_attr($settings['block']); ?>">
	...
	protected function _content_template() {
		?>
		<a href="{{ settings.button_link.url }}" class="btn btn-{{ settings.outline }}{{ settings.style }} {{ settings.size }} {{ settings.block }}">
		    <# if( settings.icon ) { #>
