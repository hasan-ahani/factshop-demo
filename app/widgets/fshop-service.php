...
class Service extends Widget_Base {
	...
	public function get_name() {
		return 'factshop-service';
	}
	
	public function get_title() {
		return __( 'آیتم خدمات', 'factshop' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::TEXT,
				'default' => __( 'لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ و با استفاده از طراحان گرافیک است.', 'factshop' ),
	...
	protected function render() {
		...
		<div class="service-item text-center">
			...
			<i class="<?php echo esc_attr($settings['icon']) ?> icon-simple"></i>
			<?php } ?>
			<?php if( $settings['title']) { ?>
			<h4 class="my-3"><?php echo esc_attr($settings['title']); ?></h4>
			<?php } ?>
	...
	protected function _content_template() {
		...
		<div class="service-item text-center">
			<# if( settings.icon ) { #>
			<i class="{{ settings.icon }} icon-simple"></i>
			<# } #>
			<# if( settings.title ) { #>
			<h4 class="my-3">{{{ settings.title }}}</h4>
			<# } #>
			<# if( settings.text ) { #>
			<p class="mb-0">{{{ settings.text }}}</p>
			<# } #>
		</div>
		<?php
	}
