...
class Price extends Widget_Base {
	...
	public function get_name() {
		return 'factshop-price';
	}
	
	public function get_title() {
		return __( 'جدول قیمت', 'factshop' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'price',
			[
				'label' => __( 'قیمت', 'factshop' ),
				'type' => Controls_Manager::TEXT,
				'default' => __( '1000', 'factshop' ),
				'placeholder' => __( 'قیمت را وارد کنید', 'factshop' )
			]
		);
		...
		$this->add_control(
			'currency',
			[
				'label' => __( 'واحد پول', 'factshop' ),
				'type' => Controls_Manager::TEXT,
				'default' => __( 'تومان', 'factshop' ),
				'placeholder' => __( 'واحد پول را وارد کنید', 'factshop' )
			]
		);
		...
		$this->add_control(
			'time',
			[
				'label' => __( 'زمان', 'factshop' ),
				'type' => Controls_Manager::TEXT,
				'default' => __( 'ماه', 'factshop' ),
			]
		);
		...
		$this->add_control(
			'button_title',
			[
				'label' => __( 'عنوان دکمه', 'factshop' ),
				'type' => Controls_Manager::TEXT,
				'default' => __( 'شروع کنید', 'factshop' ),
				'placeholder' => __( 'عنوان دکمه را وارد کنید', 'factshop' )
			]
		);
        ...
        $this->add_control(
            'button_style',
            [
                'label' => __( 'سبک', 'factshop' ),
                'type' => Controls_Manager::SELECT,
                'default' => 'primary',
                'options' => [
                    'primary'  => __( 'پیشفرض', 'factshop' ),
                    'secondary'  => __( 'طوسی', 'factshop' ),
                    'success'  => __( 'سبز', 'factshop' ),
                    'danger'  => __( 'قرمز', 'factshop' ),
                    'warning'  => __( 'زرد', 'factshop' ),
                    'info'  => __( 'فیروزه', 'factshop' ),
                    'light'  => __( 'سفید', 'factshop' ),
                    'dark'  => __( 'سیاه', 'factshop' ),
                    'link'  => __( 'لینک', 'factshop' ),
                ],
            ]
        );
		...
		$this->add_control(
			'recommended',
			[
				'label' => __( 'پیشنهادی', 'factshop' ),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => __( 'بله', 'factshop' ),
				'label_off' => __( 'خیر', 'factshop' ),
				'return_value' => 'yes',
				'default' => 'no',
			]
		);
	...
	protected function render() {
		...
		?>
		<!-- price item -->
		<div class="price-item text-center <?php if ( 'yes' === $settings['recommended'] ) { echo 'recommended'; } ?>">
			<?php if( $settings['icon'] ) { ?>
			<i class="price-icon <?php echo esc_attr($settings['icon']); ?>"></i>
			<?php } ?>
			<?php if( $settings['title'] ) { ?>
			<span class="plan"><?php echo esc_attr($settings['title']); ?></span>
			<?php } ?>
			
			<h3 class="price">
                <?php if( $settings['time'] ) { ?>
                    <span><?php echo esc_attr($settings['time']) ?>/</span>
                <?php } ?>
				<?php if( $settings['price'] ) { ?>
				<?php echo esc_attr($settings['price']); ?>
				<?php } ?>

                <?php if( $settings['currency'] ) { ?>
                    <em><?php echo esc_attr($settings['currency']); ?></em>
                <?php } ?>
			</h3>
			<?php if( $settings['content_value'] ) { ?>
			<div class="price-content">
				<?php echo wp_kses($settings['content_value'], $allowed_html) ?>
			</div>
			<?php } ?>
			<?php if ( $settings['button_link'] || $settings['button_title'] ) : ?>
			<a href="<?php echo esc_url($settings['button_link']['url']); ?>" class="btn btn-<?php echo esc_attr($settings['button_style']); ?>"><?php echo esc_attr($settings['button_title']); ?></a>
			<?php endif; ?>
		</div>
		<?php
	}
	...
	protected function _content_template() {
		?>
		<!-- price item -->
		<div class="price-item text-center <# if ( 'yes' === settings.recommended ) { #> recommended <# } #>">
			<# if( settings.icon ) { #>
			<i class="price-icon {{ settings.icon }}"></i>
			<# } #>
			<# if ( settings.title ) { #>
			<span class="plan">{{{ settings.title }}}</span>
			<# } #>
			<h3 class="price">
                <# if( settings.time ) { #>
			...
			<# if( settings.content_value ) { #>	
			<div class="price-content">
				{{{ settings.content_value }}}
			</div>
			<# } #>
			<# if( settings.button_title ) { #>
			<a href="{{ settings.button_link }}" class="btn btn-{{ settings.button_style }}">{{{ settings.button_title }}}</a>
			<# } #>
		</div>
		<?php
	}
